...
description: "A SQL critic, a profile table of contents, and medium reasoning changed behavior and cost—but did not beat the simpler setup."
pubDate: "2026-08-26"
---

**TL;DR:** Neither the profile bundle, a critic sub-agent, nor medium reasoning improved accuracy. At low effort, the profile made 66% fewer database queries but cost 19% more. Low effort matched medium while using 11% fewer tokens, 18% less money, and 23% less time. The critic added 56% more tokens and 58% more latency.

**Experiment:** The new low-effort run completed the same 498 [BIRD Mini-Dev](https://bird-bench.github.io/) questions in both arms, without the critic. This is the cleanest profile comparison, so it replaces the earlier 491-question headline result. The primary metric is **execution accuracy**: the share of generated SQL queries whose result matches the gold query. Higher is better.

I analyzed three changes to the [text-to-SQL benchmark](/blog/profiler-doesnt-help/):
...
<p class="result-note"><strong>Bottom line:</strong> the profile bundle reduced database traffic, but did not improve accuracy and made the run slower and more expensive.</p>
...
The earlier medium-effort profile arm stopped at 491 questions because I ran out of money. The low-effort rerun completed all 498 available questions in both arms.
